...
function getDecodedPassword() {
    try { 
        ...
        // Important Fix: Return null instead of empty string on error
        // to prevent unlocking with an empty input.
        return null; 
    }
}
...
function unlockPersonalLife() {
    ...
    // 2. Disable button to prevent double-click issues
    btn.disabled = true;
    
    const CORRECT_PASSWORD = getDecodedPassword();
    
    // 3. Strict Check: Ensure CORRECT_PASSWORD is not null/empty
...
function lockPersonalLife() {
    ...
    // Re-enable the button when locking
    const btn = document.querySelector('.password-gate .btn');
